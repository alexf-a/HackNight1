//VCC is power
//Grd is Ground
//DIN or digiral in Goes to Pin 12
//CS goes to pin 10
//CLK or Clock goes to pin 11
...
int pos;
void loop() {
  Serial.println("AND START");
  pos = lc.getCharArrayPosition('H');
  for (int i = 0; i < 6; i++) {
    lc.setRow(0, i, alphabetBitmap[pos][i]);
  }
  delay(1000);
  lc.clearAll();
  int pos =  lc.getCharArrayPosition('E');
  for (int i = 0; i < 6; i++) {
    lc.setRow(0, i, alphabetBitmap[pos][i]);
  }
  delay(1000);
  lc.clearAll();
  pos =  lc.getCharArrayPosition('L');
  for (int i = 0; i < 6; i++) {
    lc.setRow(0, i, alphabetBitmap[pos][i]);
  }
  delay(1000);
